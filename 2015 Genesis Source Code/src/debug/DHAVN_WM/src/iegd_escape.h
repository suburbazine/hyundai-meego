...
#define INTEL_ESCAPE_SET_CLONE_MODE	         0x20029
#define INTEL_ESCAPE_SET_DIH_MODE	         0x2002A
#define INTEL_ESCAPE_GET_DIH_CLONE_STATE     0x2002B
#define INTEL_ESCAPE_GET_DIH_CLONE_ENABLE    0x2002C
#define INTEL_ESCAPE_FREEZE_FB				 0x2002D
#define INTEL_ESCAPE_UNFREEZE_FB			 0x2002E
#define INTEL_ESCAPE_SWAPDC					 0x20030
#define INTEL_ESCAPE_SHOW_DESKTOP            0x20031
...
#define INTEL_ESCAPE_VIDEO_PLANE_CTRL_OVERRIDE	0x20105

/* Direct Rendering Related Commands */

#define INTEL_ESCAPE_MAP_CI				0x20106
#define INTEL_ESCAPE_FREE_CI				0x20107
...
#define INTEL_ESCAPE_X_SUSPEND			     0x20220

/* GL Texture Stream*/
#define INTEL_ESCAPE_GET_GL_TEXTURE_STREAM	0x20221
...
static __inline char *intel_esc_str(unsigned int esc)
{
	...
	case INTEL_ESCAPE_SET_CLONE_MODE:
		return "INTEL_ESCAPE_SET_CLONE_MODE";
	case INTEL_ESCAPE_SET_DIH_MODE:
		return "INTEL_ESCAPE_SET_DIH_MODE";
	case INTEL_ESCAPE_GET_DIH_CLONE_STATE:
		return "INTEL_ESCAPE_GET_DIH_CLONE_STATE";
	case INTEL_ESCAPE_GET_DIH_CLONE_ENABLE:
		return "INTEL_ESCAPE_GET_DIH_CLONE_ENABLE";
	case INTEL_ESCAPE_VIDEO_PLANE_CTRL_OVERRIDE:
		return "INTEL_ESCAPE_VIDEO_PLANE_CTRL_OVERRIDE";
	case INTEL_ESCAPE_FREEZE_FB:
		return "INTEL_ESCAPE_FREEZE_FB";
	case INTEL_ESCAPE_GET_GL_TEXTURE_STREAM:
		return "INTEL_ESCAPE_GET_GL_TEXTURE_STREAM";
	case INTEL_ESCAPE_MAP_CI:
		return "INTEL_ESCAPE_MAP_CI";
	case INTEL_ESCAPE_FREE_CI:
		return "INTEL_ESCAPE_FREE_CI";
	default:
...
/*
 * Structures needed for v4l2 surface allocation and free from the internal
 * memory manager.
 */
typedef struct _iegd_esc_map_ci {

	unsigned int drm_context;
	unsigned long v4l2_offset;	/*get it from v4l2_buf*/
	unsigned int width;
	unsigned int height;
	unsigned long  pitch;
	unsigned long  pixel_format;
	unsigned int map_method;	/* 1:default WA; 0 kernel*/

} iegd_esc_map_ci_t;

typedef struct _iegd_esc_map_ci_reply {
	unsigned int drm_context;
	int ret;
	unsigned int buffer_id;
	unsigned int gtt_offset;
	void *mem_handle;	/*a PVR exported handle that coule be used by user mode VA driver*/
	unsigned char *virt_addr;
} iegd_esc_map_ci_reply_t;
...
typedef struct _iegd_esc_color_params {
	...
	unsigned int colorkey_autopaint;
	unsigned int drm_context;
...
/* Input to driver with display id/number */
typedef struct _iegd_esc_display_num {
	unsigned long display_num; /* primary or secondary */
} iegd_esc_display_num_t;


typedef struct _iegd_esc_get_dih_clone_state {
	unsigned int dih_clone_state;
} iegd_esc_get_dih_clone_state;

typedef struct _iegd_esc_get_dih_clone_enable {
	unsigned long dih_clone_enable;
} iegd_esc_get_dih_clone_enable;


/*************************************************** 
 *	Overlay Plane Control Override Definitions *
 ***************************************************/
typedef enum  {
	OR_FUNC_OVERLAY_BIND, 	//0: overlay placement override
	OR_FUNC_COLOR_CORRECT, 	//1: color correction override
	OR_FUNC_FB_BLEND_OVL, 	//2: FB blend overlay ON/OFF
} override_func_t;
...
#define PRIMARY_DISPLAY_MASK 	BIT0
#define SECONDARY_DISPLAY_MASK 	BIT1

typedef struct _iegd_esc_video_plane_ctrl_override_t {
	/* 
	 *	typedef enum  {
	 *		OR_FUNC_OVERLAY_BIND, //0: overlay placement override
	 *		OR_FUNC_COLOR_CORRECT, //1: color correction override
	 ...
	 *	}override_func_t;
	 *
	 *	Bitwise OR mask to select the supported functions and allow 
	 *	1 time configuration of all custom override			*/
	unsigned int override_func; 
	/* 
	 *	typedef enum {
	 *		HW_OVERLAY,
	 *		SPRITEC,
	 *	}ovl_plane_t; 		*/
	struct {
		ovl_plane_t primary_ovl_bind;
		//Secondary will be assigned to the unused overlay plane
	} override_func_in_overlay_bind;
	struct {
		iegd_esc_color_params_t first_ovl_color_param;
		iegd_esc_color_params_t second_ovl_color_param;
		/* Bit 0 represents HW overlay
		 * Bit 1 represents SpriteC 	*/
		unsigned int color_correct_target_mask;
	} override_func_in_color_correct;
	struct {
		/* Bit 0 represents primary
		 * Bit 1 represents secondary
		 * e.g. 
		 * 1)To turn on FB blend on secondary but turn off FB blend on primary:
		 * 	dpy_primary_secondary_on = 2;
		 * 	fb_blend_target_mask = 3; 
		 * 2)To turn off FB blend on primary but leave FB blend on secondary as it is
		 *	dpy_primary_secondary_on = 0;
		 * 	fb_blend_target_mask = 1; 					*/
           unsigned int dpy_primary_secondary_on;
           unsigned int fb_blend_target_mask; 
     } override_func_in_fb_blend_ovl;
} iegd_esc_video_plane_ctrl_override_t;
