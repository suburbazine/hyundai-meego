/*
    File:       iAp2ServiceExternelLogUtils.cpp
...
void iAp2ServiceExternelLogWriteAddQueue(LogLevel inLevel, const char *inStr, int inLen) {
	struct tailq_entry * item = NULL;

	// Tail queue initialize
	if (TAILQ_EMPTY(&my_tailq_head)) TAILQ_INIT(&my_tailq_head);
...
static void * __iAp2ServiceExternelLogWriteThread( void * inArgs ) {
	(void) inArgs;
	struct tailq_entry * item = NULL;

	for(;;) {
		// Print log to Log Assistance
        while (TAILQ_FIRST(&my_tailq_head) != NULL) {
...
OSStatus iAp2ServiceExternelLogWriteInit(void)
{
	OSStatus err = 0;
	...
	if( gLogWriteThread == 0 ) {
		...
		err = pthread_create(&gLogWriteThread, NULL, __iAp2ServiceExternelLogWriteThread, NULL);
		if( err != 0) {
            goto exit;  
        }
	}
...
void iAp2ServiceExternelLogWriteStop(void)
{
	// Thread force exit.
	if (gLogWriteThread != 0) {
	...
	while (TAILQ_FIRST(&my_tailq_head) != NULL) {
		struct tailq_entry * item = TAILQ_FIRST(&my_tailq_head);
		TAILQ_REMOVE(&my_tailq_head, item, entries);
		if (item->logStr != NULL) {
                free(item->logStr);
              }
